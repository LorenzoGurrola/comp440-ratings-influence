...
Run the parts, start to finish, and say what is still missing.
    ...
    uv run python run_all.py              every core part: before you submit
    uv run python run_all.py --part 3      Parts 1 to 3 only
    uv run python run_all.py --verbose     every script's full output as well

Runs measures.py's self-check, then the core Parts 1 to 4. Part 5 is the reflection: it has slots
and no script. It never runs a follow-up: those are optional, and they run only when you ask
Claude for one. With --part N it stops after Part N, so that a later part's results never appear
before you have said what you expect from it. A part that stops, because it failed or because
what it needs is not done yet (your rule in my_choice.py, your recommender in my_recommender.py),
...
against you, so this is safe to run from the first day. The follow-up slots are listed and
never counted as missing: they are optional. With --part N it ends with every slot
