...
recommender.py) and picks by true popularity alone, because the shipped choice rule
(independent_choice in choose.py) ignores the counts (social influence 0). This is the independent
condition, the control for the later parts.

Prints the five measures over WORLDS worlds, then world 0's share for each artist, so that one
world can be read against the figure. Saves figures/part1_strip.png: one column per artist, one
dot per world.
"""
...
def main():
    shares = simulate(random_five, independent_choice, WORLDS, social_influence=0.0)
