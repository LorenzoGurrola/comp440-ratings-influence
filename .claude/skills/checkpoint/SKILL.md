---
name: checkpoint
description: The end-of-activity check. Run it once, when Parts 0 to 5 are done and before submission - or mid-activity if the student asks what is missing. Runs run_all.py, names the blank writeup slots that count, and confirms this session is in TRANSCRIPT.md.
---

The activity's six parts each end with a commit, not a checkpoint. This skill runs **once**, at
...
2. Name, one line each, every slot the output lists as still `XXXX`, and ask what each needs; do
   ...
   **The follow-up slots at the end of `WRITEUP.md` never count.** `run_all.py` prints them as
   "follow-up slot, not counted". Leave them as `XXXX` unless the student did that follow-up, and
